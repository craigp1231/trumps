...
const {
	players, rounds, dealer, roundBids, roundTricks, trickSchedule, dealerForRound,
} = storeToRefs(game)
...
function startBidding(number, tricks) {
	...
	biddingRound.value = { number, tricks, trickLimit: tricks === 'NT' ? 0 : tricks, suit: game.roundSuits[number] ?? null }
	biddingPlayers.value = order.map((name) => {
		const playerIndex = players.value.indexOf(name)
		return { name, playerIndex, bid: tricks === 'NT' ? 0 : (roundBids.value[number]?.[playerIndex] ?? null) }
	})
}
...
function chooseBid(player, bid) {
	if (player === biddingPlayers.value.at(-1) && dealerBidIsForbidden(bid)) return
	player.bid = bid
}
...
function startTricksEntry(number, tricks) {
	if (players.value.length < 2 || (!roundBids.value[number] && trickSchedule.value[number - 1] !== 'NT')) return
	if (trickSchedule.value[number - 1] === 'NT') game.saveBids(number, players.value.map(() => 0), game.roundSuits[number])
	tricksRound.value = { number, tricks, trickLimit: tricks === 'NT' ? 7 : tricks }
	tricksPlayers.value = players.value.map((name, playerIndex) => ({ name, playerIndex, tricks: roundTricks.value[number]?.[playerIndex] ?? 0 }))
}
...
<template>
	<main class="app-shell">
		<GameHeader @edit-players="openPlayerEditor" @new-game="openNewGameDialog" />
		<TrickSchedule @start-bidding="startBidding" @start-tricks="startTricksEntry" />
		<GameHistory />
		<PlayerEditorModal :open="isPlayerEditorOpen" @close="closePlayerEditor" />
		<DealerModal :open="isDealerDialogOpen" :selected-dealer="selectedDealer" @close="closeNewGameDialog" @choose="chooseDealer" />
		<BiddingModal :round="biddingRound && { ...biddingRound, players: biddingPlayers }" :dealer-bid-is-forbidden="dealerBidIsForbidden" @close="closeBidding" @choose-bid="chooseBid" @choose-suit="chooseSuit" />
		<TricksModal :round="tricksRound && { ...tricksRound, players: tricksPlayers }" @close="closeTricksEntry" @choose-tricks="(player, tricks) => player.tricks = tricks" />
	</main>
</template>
