...
<template>
	<div v-if="round" class="modal-backdrop" @click.self="$emit('close')">
		<section class="player-modal bidding-modal" role="dialog" aria-modal="true" aria-labelledby="tricks-dialog-title">
            ...
            @click="$emit('close')"
            >&#10005;</button>
			<p class="eyebrow">Round {{ round.number }} &middot; {{ round.tricks }} tricks</p>
			...
			<div class="bid-fields">
                <div v-for="player in round.players" :key="player.name" class="bid-player">
                    <span class="bid-player-name tricks-player-name">{{ player.name }}<small>Bid {{ game.bidForRound(round.number, player.playerIndex) }}</small></span>
			...
			<div class="modal-actions">
                ...
                <button class="add-button" type="button" @click="saveTricks">Save tricks</button>
            </div>
		</section>
	</div>
</template>
