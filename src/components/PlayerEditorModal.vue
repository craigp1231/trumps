...
<template>
  <div v-if="open" class="modal-backdrop" @click.self="$emit('close')">
    ...
    >
      ...
      <h2 id="player-dialog-title">Edit players</h2>
      <p class="modal-copy">
        Update the names at your table. Existing scores stay with their player
        position.
      </p>
      <div class="player-fields">
        <label v-for="(name, index) in editedPlayers" :key="index"
          >Player {{ index + 1
          }}<span class="player-input-row"
            ><input
              ref="playerInputs"
              v-model="editedPlayers[index]"
              type="text"
              maxlength="24"
            /><button
              class="remove-player"
      ...
      <div class="modal-actions">
        <button class="cancel-button" type="button" @click="$emit('close')">
          Cancel</button
        ><button class="add-button" type="button" @click="savePlayers">
          Save players
        </button>
      </div>
    </section>
